...
# Accepted GFPGAN ONNX filenames (legacy + canonical) and the public
# release URL. The loader reads input shape dynamically, so any size
# works — we download the 512 build (GFPGANv1.4.onnx) from the
# Face-Upscalers-ONNX release which ships the canonical export.
_GFPGAN_FILES = ("GFPGANv1.4.onnx", "gfpgan-1024.onnx", "gfpgan_1.4.onnx")
_GFPGAN_URL = (
    "https://github.com/harisreedhar/Face-Upscalers-ONNX/releases/"
    "download/Models/GFPGANv1.4.onnx"
)


def _resolve_gfpgan_path() -> str | None:
    for name in _GFPGAN_FILES:
...
def _ensure_gfpgan_model() -> str | None:
    path = _resolve_gfpgan_path()
    if path is not None:
        return path
    # Auto-download the canonical GFPGANv1.4 build into models/.
    from modules.utilities import conditional_download
    try:
        os.makedirs(models_dir, exist_ok=True)
        update_status(f"Downloading GFPGANv1.4.onnx (~330MB)...", NAME)
        conditional_download(models_dir, [_GFPGAN_URL])
    except Exception as e:
        update_status(f"GFPGAN download failed: {e}", NAME)
    return _resolve_gfpgan_path()
...
def pre_check() -> bool:
    if _ensure_gfpgan_model() is None:
        update_status(
            f"GFPGAN ONNX model not found in {models_dir}. "
            f"Auto-download failed — manually download {_GFPGAN_URL} and "
            "place it as GFPGANv1.4.onnx in the models folder.",
            NAME,
        )
        return False
    return True
...
def get_face_enhancer() -> onnxruntime.InferenceSession:
    ...
    with THREAD_LOCK:
        if FACE_ENHANCER is None:
            model_path = _ensure_gfpgan_model()

            if model_path is None:
                raise FileNotFoundError(
                    f"{NAME}: GFPGAN model missing in {models_dir}. "
                    f"Download {_GFPGAN_URL} into the models folder."
                )
