...
Dev mode turns on if EITHER is true:
    ...
    2. ``PHANTOMCAST_DEV_MODE`` env var is set to a truthy value (1/true/yes/on/y).
...
# Production: real backend round-trip required. Set PHANTOMCAST_DEV_MODE=1 in the
...
def is_dev_mode() -> bool:
    ...
    if TEST_MODE_ACCEPT_ANY_KEY:
        return True
    return os.environ.get("PHANTOMCAST_DEV_MODE", "").strip().lower() in _TRUTHY
