...
# Cache for paste-back. Keyed on (size, height_quant, width_quant) so the
# Forehead sliders can shape the blend region without rebuilding the mask
# every frame at the same setting.
_paste_cache: dict = {}


def _get_soft_alpha(size: int) -> np.ndarray:
    """Feathered elliptical alpha template in aligned-face space, cached.

    A square mask (the legacy approach) leaves a visible rectangular seam
    after warp-back because the corners of the aligned crop are background
    pixels, not face. An ellipse centred on the ArcFace face position —
    horizontally centred, slightly above vertical mid — covers the face
    oval and lets the corners stay as the original frame. Heavy Gaussian
    feathering then dissolves the boundary entirely.

    The Forehead sliders extend the ellipse upward (``forehead_size``) and
    laterally (``forehead_width``) so more of the head/forehead identity
    blends back onto the target instead of being clipped to the face.
    """
    h_pct = max(0.0, min(100.0, getattr(modules.globals, "forehead_size", 0.0)))
    w_pct = max(0.0, min(100.0, getattr(modules.globals, "forehead_width", 0.0)))
    # Quantize to 5% steps so we don't blow the cache during slider drags.
    h_q = int(round(h_pct / 5.0))
    w_q = int(round(w_pct / 5.0))
    key = (size, h_q, w_q)
    cached = _paste_cache.get(key)
    if cached is not None:
        return cached

    h_factor = (h_q * 5) / 100.0
    w_factor = (w_q * 5) / 100.0
    # Default ellipse covers the face oval. As h_factor → 1 the centre lifts
    # toward the brow line and the vertical radius grows, pulling the top of
    # the ellipse well above the hairline of the aligned crop. w_factor adds
    # gentle lateral spread so wider head shapes still blend cleanly.
    cy_frac = 0.52 - 0.10 * h_factor   # 0.52 → 0.42
    ay_frac = 0.55 + 0.20 * h_factor   # 0.55 → 0.75
    ax_frac = 0.46 + 0.10 * w_factor   # 0.46 → 0.56

    mask = np.zeros((size, size), dtype=np.uint8)
    cx, cy = size // 2, int(size * cy_frac)
    ax, ay = int(size * ax_frac), int(size * ay_frac)
    cv2.ellipse(mask, (cx, cy), (ax, ay), 0, 0, 360, 255, -1)
    k_blur = max(size // 8, 5)
    mask = cv2.GaussianBlur(mask, (2 * k_blur + 1, 2 * k_blur + 1), 0)

    # Cap the cache so unique (size, h, w) combinations don't grow unbounded.
    if len(_paste_cache) > 64:
        _paste_cache.clear()
    _paste_cache[key] = mask  # uint8 [0, 255] — blended via cv2 SIMD ops
    return mask
...
def swap_face(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    # mouth_mask, any feature mask, or opacity < 1 needs an unmodified original.
    ...
    needs_original = opacity < 1.0 or feature_mask_active
    if needs_original:
    ...
    # Quick lip mask — fast lip-strip paste-back when the broader
    # mouth mask isn't active. Skipped if mouth_mask covers it already.
    if quick_lip_enabled and not mouth_mask_enabled:
        from modules.processors.frame.face_masking import (
            create_quick_lip_mask as _create_quick_lip_mask,
            apply_mask_area as _apply_mask_area,
        )
        _, lip_cutout, lip_box, lip_polygon = _create_quick_lip_mask(
            target_face, original_frame
        )
        if lip_cutout is not None and lip_box != (0, 0, 0, 0):
            swapped_frame = _apply_mask_area(
                swapped_frame, lip_cutout, lip_box, shared_face_mask, lip_polygon
            )

    # Eyes mask — keeps original eyes for sharp, expressive gaze.
    if eyes_mask_enabled:
        from modules.processors.frame.face_masking import (
            create_eyes_mask as _create_eyes_mask,
            apply_mask_area as _apply_mask_area,
        )
        _, eyes_cutout, eyes_box, eyes_polygon = _create_eyes_mask(
            target_face, original_frame
        )
    ...
    if eyebrows_mask_enabled:
        from modules.processors.frame.face_masking import (
            create_eyebrows_mask as _create_eyebrows_mask,
            apply_mask_area as _apply_mask_area,
        )
        _, brow_cutout, brow_box, brow_polygon = _create_eyebrows_mask(
            target_face, original_frame
        )
    ...
    if chin_mask_enabled:
        from modules.processors.frame.face_masking import (
            create_chin_mask as _create_chin_mask,
            apply_mask_area as _apply_mask_area,
        )
        _, chin_cutout, chin_box, chin_polygon = _create_chin_mask(
            target_face, original_frame
        )
    ...
    try: # Wrap main logic in try-except
        ...
        if eyebrows.shape[0] > 0:
            ...
            if norm > 0:
                ...
                # Forehead sliders scale how far up and how wide the
                # forehead points reach. At 0 we keep the legacy 1.0×
                # vertical / 1.2× lateral expansion. At 100 we extend
                # well past the hairline so Poisson blending and mouth
                # masking treat the whole head as the target region.
                fh_pct = max(0.0, min(100.0, getattr(modules.globals, "forehead_size", 0.0)))
                fw_pct = max(0.0, min(100.0, getattr(modules.globals, "forehead_width", 0.0)))
                vertical_mult = 1.0 + 0.6 * (fh_pct / 100.0)   # 1.0 → 1.6
                lateral_mult = 1.2 + 0.4 * (fw_pct / 100.0)    # 1.2 → 1.6

                forehead_offset = up_vector * (norm * vertical_mult)

                # Shift eyebrows up to create forehead points
                forehead_points = eyebrows + forehead_offset

                # Expand the top points outwards to cover forehead corners
                top_center = np.mean(forehead_points, axis=0)
                forehead_points = (forehead_points - top_center) * lateral_mult + top_center
