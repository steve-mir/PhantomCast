...
exports.cleanupExpiredActivations = exports.v1_nowpayments = exports.v1_createSubscription = exports.v1_subscriptionStatus = exports.v1_paymentStatus = exports.v1_createPayment = exports.v1_modelUrl = exports.v1_moveLicense = exports.v1_deactivate = exports.v1_heartbeat = exports.v1_activate = void 0;
/**
...
const PHANTOMCAST_SIGNING_PRIVATE_KEY = (0, params_1.defineSecret)("PHANTOMCAST_SIGNING_PRIVATE_KEY"); // RS256 PEM
const NOWPAY_API_KEY = (0, params_1.defineSecret)("NOWPAY_API_KEY");
const NOWPAY_IPN_SECRET = (0, params_1.defineSecret)("NOWPAY_IPN_SECRET");
...
function planFromNowpayId(planId) {
    if (!planId)
        return undefined;
    if (planId === NOWPAY_PLAN_PRO)
        return "pro";
    if (planId === NOWPAY_PLAN_STUDIO)
        return "studio";
    return undefined;
}
// ---------- Helpers ----------
...
function effectiveFeatures(data) {
...
function similarityScore(stored, current) {
    let score = 0;
    for (const [k, w] of Object.entries(COMPONENT_WEIGHTS)) {
        if (stored[k] && current[k] && stored[k] === current[k])
            score += w;
    }
    return score;
}
function signClaims(payload) {
    ...
    const token = jwt.sign({ ...payload, iat, exp }, PHANTOMCAST_SIGNING_PRIVATE_KEY.value(), { algorithm: "RS256", header: { alg: "RS256", kid } });
    return { token, exp };
}
async function audit(action, ok, ctx) {
...
// ---------- v1_activate ----------
exports.v1_activate = (0, https_1.onRequest)({ region: "us-central1", cors: false, secrets: [PHANTOMCAST_SIGNING_PRIVATE_KEY] }, async (req, res) => {
    if (req.method !== "POST") {
    ...
    if (!license_key || !fingerprint) {
        res.status(400).json({ error: { code: "bad_request", message: "Missing fields." } });
        return;
    }
...
// ---------- v1_heartbeat ----------
exports.v1_heartbeat = (0, https_1.onRequest)({ region: "us-central1", secrets: [PHANTOMCAST_SIGNING_PRIVATE_KEY] }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "method" });
        return;
    }
    ...
    if (!snap.exists) {
        res.status(401).json({ error: { code: "license_invalid" } });
        return;
    }
    ...
    if (data.status !== "active") {
        res.status(401).json({ error: { code: "license_suspended", message: data.status } });
        return;
    }
    const endMs = periodEndMs(data.currentPeriodEnd);
    if (endMs !== null && endMs < Date.now()) {
    ...
    if (data.fingerprintHash && fingerprint && data.fingerprintHash !== fingerprint) {
        res.status(409).json({ error: { code: "device_mismatch" } });
        return;
    }
...
// ---------- v1_deactivate ----------
exports.v1_deactivate = (0, https_1.onRequest)({ region: "us-central1" }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "method" });
        return;
    }
...
// ---------- v1_moveLicense ----------
exports.v1_moveLicense = (0, https_1.onRequest)({ region: "us-central1" }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "method" });
        return;
    }
...
exports.v1_modelUrl = (0, https_1.onRequest)({ region: "us-central1" }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "method" });
        return;
    }
    ...
    if (required && plan !== required && plan !== "studio") {
        res.status(403).json({ error: { code: "plan_required", message: required } });
        return;
    }
...
const PLAN_MONTHLY_USD = { pro: 19, studio: 49 };
// ---------- v1_createPayment: $910 activation invoice ----------
exports.v1_createPayment = (0, https_1.onRequest)({
    region: "us-central1",
    cors: true,
    ...
    const { plan, email, pay_currency } = (req.body ?? {});
    ...
    res.status(200).json({
        ...
        plan,
        monthly_after_grace: PLAN_MONTHLY_USD[plan],
    });
});
const FUNCTIONS_REGION_HOST = "us-central1-diivix1.cloudfunctions.net";
// ---------- v1_paymentStatus: poll for the issued key after payment ----------
exports.v1_paymentStatus = (0, https_1.onRequest)({ region: "us-central1", cors: true }, async (req, res) => {
    if (req.method === "OPTIONS") {
        res.status(204).send("");
        return;
    }
...
exports.v1_subscriptionStatus = (0, https_1.onRequest)({ region: "us-central1", cors: true }, async (req, res) => {
    if (req.method === "OPTIONS") {
        res.status(204).send("");
        return;
    }
    ...
    const { license_id, license_key } = (req.body ?? {});
    ...
    res.status(200).json({
        ...
        monthly_after_grace: PLAN_MONTHLY_USD[data.plan] ?? null,
...
let nowpayJwt = null;
async function nowpaymentsBearerToken() {
    if (!NOWPAY_EMAIL || !NOWPAY_PASSWORD) {
        throw new Error("nowpayments_creds_missing");
    }
    const now = Math.floor(Date.now() / 1000);
    if (nowpayJwt && nowpayJwt.exp - 30 > now)
        return nowpayJwt.token;
    ...
    if (!r.ok)
        throw new Error(`nowpayments auth failed: ${r.status}`);
    const j = await r.json();
    // NOWPayments JWTs are valid ~5 minutes; cache conservatively for 4.
    nowpayJwt = { token: j.token, exp: now + 4 * 60 };
    return j.token;
}
async function nowpaymentsCreateEmailSubscription(planId, email, orderId) {
...
exports.v1_createSubscription = (0, https_1.onRequest)({
    region: "us-central1",
    secrets: [NOWPAY_API_KEY],
}, async (req, res) => {
    if (req.method !== "POST") {
...
exports.v1_nowpayments = (0, https_1.onRequest)({
    region: "us-central1",
    cors: false,
    ...
    const subscriptionId = body.subscription_id ?? body.parent_payment_id;
    ...
    const seenPayments = existing.processedPaymentIds ?? [];
...
// ---------- scheduled: cleanup ----------
exports.cleanupExpiredActivations = (0, scheduler_1.onSchedule)({ region: "us-central1", schedule: "every 24 hours" }, async () => {
    const now = new Date();
